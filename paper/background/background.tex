...
\renewcommand{\CSTLCdrulename}[1]{{\tiny \text{#1}}}
\renewcommand{\CHSTLCdrulename}[1]{{\scriptsize \text{#1}}}
%\renewcommand{\CSTLCdrulename}[1]{{\scriptsize \text{#1}}}
\renewcommand{\CoCdrulename}[1]{\normalfont\scriptsize \text{#1}}
\renewcommand{\CoCSdrulename}[1]{\normalfont\scriptsize \text{#1}}
\renewcommand{\Fdrulename}[1]{\normalfont\scriptsize \text{#1}}
\renewcommand{\Fwdrulename}[1]{\normalfont\scriptsize \text{#1}}
\renewcommand{\SSFdrulename}[1]{\normalfont\scriptsize \text{#1}}
\renewcommand{\Tdrulename}[1]{\normalfont\scriptsize \text{#1}}
\renewcommand{\Combdrulename}[1]{\normalfont\scriptsize \text{#1}}
\renewcommand{\DCdrulename}[1]{\normalfont\scriptsize \text{#1}}
\renewcommand{\LBMMTdrulename}[1]{\normalfont\scriptsize \text{#1}}
\renewcommand{\Lamdrulename}[1]{\normalfont\scriptsize \text{#1}}
\renewcommand{\Lamddrulename}[1]{\normalfont\scriptsize \text{#1}}
\renewcommand{\Lamudrulename}[1]{ {\normalfont\scriptsize \text{#1}} }
\renewcommand{\TTdrulename}[1]{\normalfont\scriptsize \text{#1}}
...
In fact there are three perspectives of computation\index{Three
  Perspectives of Computation}:
\begin{center}
  \begin{tikzpicture}       
...
relationship is the Curry-Howard correspondence\index{Curry-Howard
  Isomorphism} (or isomorphism) or the proofs-as-programs
propositions-as-types correspondence\index{Proofs-as-programs
...
were not the only ones.  Hence, this former name is
...
Harper calls this connection the ``Holy Trinity''\index{Holy Trinity}
...
Intuitionism\index{Intuitionism} began with Luitzen Brouwer.  Implicit
...
BHK-interpretation\index{BHK-interpretation} of intuitionistic logic.
...
the types of the combinatory logic\index{Combinatory Logic} correspond
...
requires advanced mathematical tools\index{Logical Consistency}.  In
...
Automath\index{Automath}.  Automath was a formal language much like a
type theory devised by Nicolaas de Bruijn in the late sixties.  A
large body of ideas in modern type theory came from Automath.  It
allowed for the specification of complete mathematical theories and
was equipped with a automated proof checker which was used to check
the correctness of the formalized theories.  In fact Automath can be
thought of as the grandfather to dependent type theory.  It was a
wonderful line of work that resulted in a large number of great ideas.
One important thing was that de Bruijn independently from Howard
stated the correspondence between intuitionistic propositional logic
and type theory \cite{Sorensen:2006}.
...
There are some beautiful dualities present in classical logic\index{Duality}.  We say
...
involution then $i(i(x)) = x$\index{Involution}.  Now in classical logic negation is
...
``nondependent types''\index{Nondependent Types}.  These are types
...
logic\index{Dependent Types}.  As we mentioned above, the first
...
theory''\index{Dependent Type Theory}.  Since the work of de Bruijn a large body of research on
...
\chapter{Dependent Types in Practice}
...
theory\index{Dependent Type Theory}.  One of the first is called NuPrl which is based on
...
Martin-L\"of's Type Theory called LF \cite{Pfenning:1999}\index{LF}.  More
...
and add general recursion and $\mathsf{Type:Type}$\index{Type:Type}.  Then, either
...
The language they use is called $\text{Sep}^3$\index{Separation of
  Proof from Program ($\text{Sep}^3$)} which stands for
...
propositions.  This is called freedom of speech\index{Freedom of Speech}.  It turns out that
...
\cite{Mishra-Linger:2008}.  The erasure\index{Erasure} was investigated in a similar
...
programming\index{Generic Programming}.
...
\begin{definition}
  \label{def:weak_norm}
  A type theory is \emph{weakly normalizing}\index{Weak Normalization} if and only for all terms $t$
...
\begin{definition}
  ...
  A type theory is \emph{strongly normalizing}\index{Strong Normalization} or \emph{terminating}
...
example, the simply-typed $\lambda$-calculus, predicative polymorphic\index{Predicativity}
versions of system F\index{Girard/Reynolds System F}, and the LF\index{LF} family of dependent types have all
...
T\index{G\"odel's System T} using hereditary substitution, because the usual ordering on types
...
an ordering is non-trivial to construct.  Furthermore, impredicative\index{Impredicativity}
...
Hereditary substitution can be used to maintain canonical
forms\index{Canonical Form} and
...
much like system $\Fw$\index{Type-level computation}\index{System $\Fw$}.  In that paper the authors were investigating
...
William Tait.  He showed strong normalization\index{Strong Normalization} of system
T\index{G\"odel's System T}using an
...
against popular belief\footnote{It has been said that while Girard was
  ...
  impossible endeavor.}, extended Tait's method\index{Tait's Method} to be able to prove
system F strongly normalizing.  He called his method reducibility
candidates. The reducibility candidates\index{Reducibility Candidates} method is based on second order
...
the disjoint union type\index{Disjoint Union Types} are equivalent in \cite{Pitts:1998}.  Karl
...
for terms.  The equivalence problem\index{Equivalence Problem} is being able to decide
...
candidates\index{Reducibility Candidates} gives rise to logical relations.  However, we have used
...
Adding the ability to define general recursive\index{General Recursion} types to a type theory
...
equivalence\index{Contextual Equivalence} of
...
types\index{Call-by-value Reduction (CBV)} and then try
