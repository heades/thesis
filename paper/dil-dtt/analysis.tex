...
translated to a derivable sequent of DIL.  Due to space constraints
the full definition of L is recalled only in the companion report
\cite{Eades:2014}. We will call a sequent in L a L-sequent and a
sequent in DIL a DIL-sequent. Along the way, we will see admissibility
of the analogs of the rules we mentioned in
...
\begin{center}
  ...
  \end{math}
\end{center}
These two rules are required for the following crucial lemma.
\begin{lemma}[Refocus]
  \label{lemma:refocus}
...
\end{lemma}
  ...
  \end{proof}
There are a number of other derived rules that the results in this
section depend on, all of which, can be found in the companion report
\cite{Eades:2014}.
      ...
      \begin{center}
        \begin{math}
          ...
          \inferrule* [right=\ifrName{andBar}] {
            [[Gr,Gr1,Gr' ; G , bar p A ! bar d @ n |- p A ! d @ n]]
      ...
      \begin{center}
        \begin{math}
          ...
          \inferrule* [right=\ifrName{axCut}] {
            [[p T' @ n' in G && Gr ; G, bar p T @ n |- bar p T' @ n']]
          }{[[Gr; G |- p T @ n]]}
        \end{math}
      \end{center}
      ...
      \begin{center}
        \begin{math}
          ...
          \inferrule* [right=\ifrName{axCutBar}] {
            [[bar p T' @ n' in G && Gr ; G , bar p T @ n |- p T' @ n']]
          }{[[Gr; G |- p T @ n]]}
        \end{math}
      \end{center}
...
\begin{lemma}[General Monotonicity]
  \label{lemma:genmono}
...
The proofs of the derived rules MonoL and MonoR depend on the
...
have to choose a particular focusing, but this is quite difficult.
Instead we show that all possible focusings in DIL of a sequent L are
derivable.  
...
the focused formula, are translated into negative hypotheses.  The
...
requires a particular formula as the focus.  We define such a
translation in the following definition.
\begin{definition}
  \label{def:focusing}
  A focusing of a derivable L-sequent $<<G |- Gr D>>$ is a DIL-sequent
  $[[-Gr-;{G}+,{D1,D2}- |- +{A}@n]]$, where $<<D>> = <<D1,n : A,D2>>$.
\end{definition}
Finally, the following theorem is the main result showing that any
focusing of a derivable L-sequent is derivable in DIL.
\begin{thm}[Containment of L in DIL]
  \label{thm:completeness}
  If $[[-Gr- ; G' |- + A @ n]]$ is a focusing of the derivable L-sequent 
...
\end{thm}
  ...
  \item[Case.]\ \\ 
    \begin{center}
      ...
      \end{math}
    \end{center}
    We know by the induction hypothesis that every focusing of $<<G |- h(Gr,(n,n)) D>>$ is derivable.  
    Suppose that $[[-Gr,(n,n)- ; G' |- + A @ n]]$ is an arbitrary focusing, where
    $[[{D}-]] \equiv [[{D1}-,-A@n,{D2}-]]$ and $[[G']] \equiv [[{D1}-,{D2}-]]$.
    This is equivalent to $[[-Gr-, n <=+ n ; G' |- + A @ n]]$, and 
    by the derived rule for reflexivity (Lemma~\ref{lemma:reflexivity}) we have 
    $[[-Gr- ; G' |- + A @ n]]$.
  ...
  \item[Case.]\ \\ 
    \begin{center}
      ...
      \end{math}
    \end{center}
    We know by the induction hypothesis that every focusing of $<<G |- h(Gr,(n1,n3)) D>>$ is derivable.  
    Suppose that $[[-Gr,(n1,n3)- ; G' |- + A @ n]]$ is an arbitrary focusing, where
    ...
    by the derived rule for transitivity (Lemma~\ref{lemma:transitivity}) we have $[[-Gr- ; G' |- + A @ n]]$,
  ...
  \item[Case.]\ \\ 
    \begin{center}
      ...
      \end{math}
    \end{center}
    It suffices to show that every focusing of $<<G, n : A |- Gr n : A , D>>$ is derivable.  Clearly,
    $[[-Gr-; {G}+,+{A}@n,{D}- |- + {A}@n]]$ is a focusing of $<<G, n : A |- Gr n : A , D>>$.  In addition,
    it is derivable:
    \begin{center}
      \begin{math}
        $$\mprset{flushleft}
    ...
    is derivable by Lemma~\ref{lemma:exchange}.

    Now consider any other focusing $[[-Gr- ; G' |- + B @ n']]$.  It must be the case that 
    $[[G']] = [[h(h(h({G}+ , + A @ n ) , {D1}-), -{A} @ n ),{D2}-]]$ for some $<<D1>>$ and $<<D2>>$.
    This sequent is then derivable as follows:
    \begin{center}
      \begin{math}
        $$\mprset{flushleft}
        \inferrule* [right=Refocus] {
          $$\mprset{flushleft}
          \inferrule* [right=Exchange] {
            $$\mprset{flushleft}
          \inferrule* [right=Ax] {
  ...
  \item[Case.]\ \\ 
    ...
    We know by the induction hypothesis that all focusings of $<<G , n1 : A , n2 : A |- Gr D>>$ are
    derivable.  Suppose $[[-Gr- ; G' |- + B @ n]]$ is an arbitrary focusing.  Then it must be the case
    that $[[G']] \equiv [[{G}+, +A@n1,+A@n2,{D1}-,{D2}-]]$, where $[[{D}-]] \equiv [[{D1}-,-B@n,{D2}-]]$.
    Now we apply the monoL derived rule (Lemma~\ref{lemma:monol}) to obtain 
    $[[-Gr- ; {G}+, +A@n1,{D1}-,{D2}- |- + B @ n]]$, which is an arbitrary focusing of 
    $<<G , n1 : A |- Gr D>>$.
  ...
  \item[Case.]\ \\ 
    ...
    all focusings of $<<G |- Gr n1 : A , n2 : A, D>>$ are derivable.  In particular, the focusing
    ...
    the monoR derived rule (Lemma~\ref{lemma:monor}).  Finally, any other focusing of $<<G |- Gr n2 : A, D>>$
    can be recused into $[[-Gr-; {G}+, {D}- |- + A @ n2]]$ (Lemma~\ref{lemma:refocus}).  Thus,
  ...
  \item[Case.]\ \\ 
    \begin{center}
      ...
      \end{math}
    \end{center}
    We know by the induction hypothesis that all focusings of $<<G |- Gr D>>$
    are derivable.  Suppose $[[-Gr- ; G' |- + A @ n]]$ is an arbitrary focusing of 
    $<<G |- Gr D>>$.  Then it must be the case that $[[G']] = [[{G}+,{D1}-,{D2}-]]$, where
    $[[{D}-]] \equiv [[{D1}-,-A@n,{D2}-]]$.  Now by weakening (Lemma~\ref{lemma:weakening}) we know 
    $[[-Gr- ; G', + < + > @ n' |- + A @ n]]$, and by exchange (Lemma~\ref{lemma:exchange})
    $[[-Gr- ; + < + > @ n', G' |- + A @ n]]$, which is exactly an arbitrary focusing of
    $<<G , n' : True |- Gr D>>$.
  ...
  \item[Case.]\ \\ 
    \begin{center}
      ...
      \end{math}
    \end{center}
    It suffices to show that every focusing of $<<G |- Gr n : True, D>>$ is derivable.
    Consider the focusing $[[-Gr- ; {G}+ , {D}- |- + {True} @ n]]$.  This is easily derivable
    by applying the \ifrName{unit} rule.  Now any other focusing of $<<G |- Gr n : True, D>>$ can
    be refocused by Lemma~\ref{lemma:refocus} into $[[-Gr- ; {G}+ , {D}- |- + {True} @ n]]$, and hence,
    are derivable.
  ...
  \item[Case.]\ \\ 
    \begin{center}
      ...
      \end{math}
    \end{center}
    Suppose $[[-Gr-; {G}+, +{False}@n, {D1}-,{D2}- |- + A @ n']]$ is an arbitrary focusing of 
    ...
    \begin{center}
      ...
      \end{math}
    \end{center}
    In the previous derivation $[[G']] \equiv [[{G}+, +<->@n, {D1}-,{D2}-]]$.  Thus, any focusing of $<<G, n : False |- Gr D>>$ is 
    derivable.
  ...
  \item[Case.]\ \\ 
    ...
    and by refocusing (Lemma~\ref{lemma:refocus}) $[[-Gr- ; G', - A @ n |- + < - > @ n' ]]$, which --
  ...
  \item[Case.]\ \\ 
    \begin{center}
      ...
      \end{math}
    \end{center}
    We know by the induction hypothesis that all focusings of $<<G , n : T1, n : T2 |- Gr D>>$
    ...
    by refocusing $[[-Gr- ; {G}+, {D1}-,{D2}-, +T1@n, - A @ n' |- - T2@n]]$, and finally by one more application
    of exchange $[[-Gr- ; {G}+, {D1}-, {D2}-,- A @ n', +T1@n |- - T2@n]]$.  At this point we know 
    $[[-Gr- ; {G}+, {D1}-, {D2}-, - A @ n' |- - T1 /\+ T2 @ n]]$ by the $\ifrName{andBar}$ rule.  Now using refocusing 
  ...
  \item[Case.]\ \\ 
    \begin{center}
      ...
      \end{math}
    \end{center}
    We know by the induction hypothesis that all focusings of $<<G |- Gr n : A, D>>$ and
    $<<G |- Gr n : B, D>>$ are derivable.  In particular, $[[-Gr- ; {G}+,{D}- |- +A @ n]]$
    and $[[-Gr- ; {G}+,{D}- |- +B @ n]]$ are derivable. Now by applying the $\ifrName{and}$ rule
    we obtain $[[-Gr- ; {G}+,{D}- |- + A /\+ B @ n]]$, which is a particular focusing of 
    $<<G |- Gr n : A /\ B, D>>$. Finally, consider any other focusing, then that focusing can be
    refocused into $[[-Gr- ; {G}+,{D}- |- + A /\+ B @ n]]$ using Lemma~\ref{lemma:refocus}, and hence,
    is derivable. Thus, we obtain our result.
  ...
  \item[Case.]\ \\ 
    \begin{center}
      ...
      \end{math}
    \end{center}
    We know by the induction hypothesis that all focusings of $<<G, n : A |- Gr D>>$ and
    $<<G, n : B |- Gr D>>$ are derivable.  So suppose $[[-Gr-; {G}+,+{A}@n,{D'}- |- + C @ n']]$ and
    $[[-Gr-; {G}+,+{B}@n,{D'}- |- + E @ n'']]$ are particular focusings, 
    where $[[{D}-]] \equiv [[{D1}-,-C@n',{D2}-,-E@n'',{D3}-]]$, and $[[{D'}-]] \equiv [[{D1}-,{D2}-,{D3}-]]$.
    By exchange (Lemma~\ref{lemma:exchange}) we know \\
    $[[-Gr-; {G}+,{D'}-,+{A}@n |- + C @ n']]$ 
    and  $[[-Gr-; {G}+,{D'}-,+{B}@n |- + E @ n'']]$.  Now by refocusing (Lemma~\ref{def:focusing}) we know 
  ...
  \item[Case.]\ \\ 
    \begin{center}
      ...
      \end{math}
    \end{center}   
    We know by the induction hypothesis that all focusings of $<<G |- Gr y : T1, D>>$ and 
    ...
    (Lemma~\ref{lemma:exchange}) and refocusing (Lemma~\ref{lemma:refocus}). We know $<<n1 Gr n2>>$ by assumption
    and so by Lemma~\ref{??} $[[-Gr- |- n1 <=+* n2]]$.  Thus, by applying the \ifrName{impBar} rule we obtain
    $[[-Gr-;{G}+,{D}- |- - {T1}->+{T2} @ n1]]$.  At this point we can refocuse the previous sequent into any
    focusing of $<<G , n1 : T1 -> T2 |- Gr D>>$, thus we obtain our result.
  ...
  \item[Case.]\ \\ 
    ...
    that all focusings of $<<G, y : T1 |- h(Gr U {(x,y)}) y : T2, D>>$ are derivable.  In particular,
    ...
    we obtain $[[-Gr- ; {G}+, {D}- |- + {T1} ->+ {T2}@n1]]$, which is a particular focusing of 
    ...
    Lastly, any other focusing of $<<G |- Gr n1 : T1 -> T2, D>>$ can be refocused
    into $[[-Gr- ; {G}+, {D}- |- + {T1} ->+ {T2}@n1]]$, and hence is derivable.
  ...
  \item[Case.]\ \\ 
    ...
    that all focusings of $<<G, n1 : T1 |- h(Gr U {(n1,n2)}) n1 : T2, D>>$ are derivable.  In particular,
    $[[-Gr-,n1 <=+ n2 ; {G}+, +{T1}@n1,{D}- |- +{T2}@n1]]$ is derivable.  By exchange (Lemma~\ref{lemma:exchange})
    $[[-Gr-,n1 <=+ n2 ; {G}+, {D}-,+{T1}@n1 |- +{T2}@n1]]$ is derivable.  Now by refocusing we know 
    $[[-Gr-,n1 <=+ n2 ; {G}+, {D}-,-{T2}@n1 |- -{T1}@n1]]$, and by assumption we know $<<y notin Gr, G, D>>$ which implies
    $[[n1 notin |-Gr-|,|{G}+,{D}-|]]$.  Thus, by applying the $\ifrName{imp}$ rule we know  
    $[[-Gr-,n1 <=+ n2 ; {G}+, {D}- |- - {T2} ->- {T1}@n2]]$ is derivable.  Clearly, this is a particular refocusing of
    any focusing of $<<G, n2 : T1 - T2 |- Gr D>>$, and any other focusing can be refocused into 
    $[[-Gr-,n1 <=+ n2 ; {G}+, {D}- |- - {T2} ->- {T1}@n2]]$, and hence are derivable.
  ...
  \item[Case.]\ \\ 
    ...
    except the particular focusing of $<<G, y : T2 |- Gr D>>$ has to be refocused so
    as to allow for the application of $\ifrName{impBar}$.
  \end{itemize}
\end{proof}
