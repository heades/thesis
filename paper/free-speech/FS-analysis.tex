...
\begin{proof}
  ...
  \item[Case.]\ \\
    ...
    using the assumption $\Gamma \tvdash{\theta} v:e_1$, the injection rules $\FSdrulename{InjDom}$ and $\FSdrulename{InjRan}$, 
    and $\FSdrulename{Conv}$. Thus, by substitution for typing (Lemma~\ref{lemma:substitution}), $\Gamma \tvdash{L}[v/x]e':[v/x]e_2$.
...
\begin{lemma}[$[[Type]]:[[Type]]$ is not Logical]\index{Freedom of
...
In order to prove that the \FSdrulename{Join} and \FSdrulename{Conv}\index{Equality}
