...
main results consist of logical consistency\index{Logical Consistency} of the logical fragment
and type preservation\index{Type Preservation}.  The former is shown by proving weak
normalization\index{Weak Normalization} of the logical fragment -- the reader may wish to recall
...
\begin{lemma}[Weakening]\index{Freedom of Speech!Weakening}
...
\begin{proof}  
  ...
  \item[Case.]\ \\
    ...
    \item[Case.]\ \\
      ...
      $\FSdrulename{\normalsize K\_Eq}$ to obtain $\Gamma,[v/x]\Gamma' \tvdash{L} [v/x]e' = [v/x]e'':[[Type]]$,
  ...
  \item[Case.]\ \\
     ...
     and then reapplying the $\FSdrulename{\normalsize Var}$ rule.  Now suppose $[[x == y]]$.  Then the
...
fragment $[[Type]] : [[Type]]$\index{Type:Type} does not hold, thus if $[[e]]$ is
...
proof.  
\begin{lemma}[Regularity]\index{Freedom of Speech!Regularity}
...
\begin{proof}
  ...
  \item[Case.] \ \\
    ...
    $\FSdrulename{\normalsize K\_Pi}$, $\Gamma \tvdash{\theta'} \arrowT{x}{\theta}{e_2}{+}{e_3}:[[Type]]$.
  ...
  \item[Case.] \ \\
    ...
    by $\FSdrulename{\normalsize K\_Pi}$ using $\Gamma, x:^L \nat \tvdash{L} e_2:[[Type]]$ and
    $\FSdrulename{\normalsize K\_Nat}$, $\Gamma \tvdash{L} \arrowT{x}{L}{\nat}{+}{e_2}:[[Type]]$.

  \item[Case.] \ \\
    \begin{center}
      \begin{math}
        $$\mprset{flushleft}
        \inferrule* [right=\ifrName{Rec}] {
          [[G |- e1 : Type th']]
          \\\\
    ...
    Now by $\FSdrulename{\normalsize K\_Pi}$, $\Gamma, f:^\theta \arrowT{x}{\theta'}{e_1}{+}{e_2} 
...
\begin{lemma}[Inversion]\index{Freedom of Speech!Inversion}
...
\begin{proof}
  ...
  \item[Case.]\ \\
    ...
    \item[i.] 
      ...
      applying the $\FSdrulename{\normalsize Conv}$ typing rule.
      ...
      \tvdash{L} e'':e'_1 = e''_1$ we can apply $\FSdrulename{\normalsize Conv}$
    ...
    \item[ii.] Here $[[e' == rec f x v]]$ and $[e''_1/y]e'_2 \equiv \arrowT{x}{L}{Nat}{+}{e}$.  
      ...
      so we can apply $\FSdrulename{\normalsize Conv}$ to obtain $\Gamma \tvdash{L} p:(\arrowT{x}{\theta}{[e''_1/y]r}{+}{[e''_1/y]s}) = (\arrowT{x}{\theta}{a}{+}{b})$.
...
\begin{thm}[Preservation]\index{Freedom of Speech!Type
    Preservation}\index{Type Preservation}
...
\begin{proof}
  ...
  \item[Case.]\ \\
    ...
    $\Gamma,x:^\theta e'_1 \tvdash{L} e':e'_2$, and $[[G |- p : ( x : th e1) ep -> e2 = ( x : th e'1) ep -> e'2 L]]$.  
    Now we may conclude $\Gamma,x:^\theta a \tvdash{L} e':e_2$ and $\Gamma \tvdash{\theta} v:a$
    using the assumption $\Gamma \tvdash{\theta} v:e_1$, the injection rules $\FSdrulename{\normalsize InjDom}$ and $\FSdrulename{\normalsize InjRan}$, 
    and $\FSdrulename{\normalsize Conv}$. Thus, by substitution for typing (Lemma~\ref{lemma:substitution}), $\Gamma \tvdash{L}[v/x]e':[v/x]e_2$.

    The third case follows similarly to the previous case.  First,
    apply inversion (Lemma~\ref{lemma:inversion}) to the typing
    assumption for $[[e]]$ followed by conversion using the injection
    rules and the conversion rules, and then finally apply
    substitution for typing (Lemma~\ref{lemma:substitution}).
  ...
  \item[Case.]\ \\
    ...
    $\FSdrulename{\normalsize AppAllTerm}$, $\Gamma \tvdash{L} e':[v/x]e_2$.
  ...
  \item[Case.]\ \\
    ...
    By applying $\FSdrulename{\normalsize Conv}$, $\Gamma \tvdash{\theta} e'':[e'_1/x]e_2$.
  ...
  \item[Case.]\ \\
    ...
    $\FSdrulename{\normalsize Coerce}$, $\Gamma \tvdash{P} e':e_1$.
...
proposed by Girard.  For an overview of the
reducibility\index{Reducibility Candidates} method see
Section~\ref{sec:tait-griard_reduciblity}.  We begin with the
definition of the interpretation of types.

\begin{definition}\index{Freedom of Speech!Interpretation of
    Types}\index{Interpretation of Types}
  \label{def:FS-interp}
...
Note that the interpretation of types depends heavily on the typing
...
interpretation of types depends on the notion of well-formed
...
$[[Type]]:[[Type]]$\index{Type:Type} axiom is purely programmatic.  The following two
lemmata establish this fact:
\begin{lemma}\index{Freedom of
    Speech!Type:Type is not Logical}
...
\begin{proof}
  ...
  $\FSdrulename{\normalsize AppAllTerm}$ or $\FSdrulename{\normalsize Conv}$, but both of
...
\begin{lemma}[Characterization of Logical Types]\index{Freedom of
    Speech!Characterization of Logical Types}
...
We now state and prove the critical properties of the interpretation
of types.  These are standard, and so we simply list them with their
proofs.

\begin{lemma}[$\CRI$]\index{Freedom of Speech!$\CRI$}
  \label{lemma:cri}
...
\begin{lemma}[$\CRII$]\index{Freedom of Speech!$\CRII$}
  \label{lemma:crii}
...
\begin{lemma}[$\CRIII$]\index{Freedom of Speech!$\CRIII$}
  \label{lemma:criii}
...
\begin{lemma}[Substitution Distribution]\index{Freedom of
    Speech!Substitution Distribution}
...
%   \end{itemize}
% \end{lemma}
\begin{lemma}[Well-formed substitution for typing]\index{Freedom of
    Speech!Well-formed Substitution for Typing}
  \label{lemma:substitution_kinding_typing}
...
\begin{proof}
  ...
  \item[Case.] \ \\
    ...
    $\FSdrulename{\normalsize Var}$ rule we obtain $\Gamma' \tvdash{\theta} x:^{\theta} \rho\,e$ which is equivalent
    to $\Gamma' \tvdash{\theta} \rho\,x:^{\theta} \rho\,e$.
  \end{itemize}
  \end{changemargin}
\end{proof}

In order to prove that the \FSdrulename{\normalsize Join} and \FSdrulename{\normalsize Conv}\index{Equality}
...
\begin{lemma}[Computational Semantic Conversion]\index{Freedom of
    Speech!Computational Semantic Conversion}
...
\begin{proof}
  ...
  by the definition of the interpretation of types.
  By $\FSdrulename{\normalsize Join}$, $\Gamma \tvdash{L} join:e = e'$. Finally, by 
  $\FSdrulename{\normalsize Conv}$, 
...
\begin{lemma}[Logical Semantic Conversion]\index{Freedom of
    Speech!Logical Semantic Conversion}
...
\begin{proof}
  ...
  accomplished by first applying $\FSdrulename{\normalsize Join}$ to obtain $\cdot \tvdash{L} join:e = e'$ 
  and then applying $\FSdrulename{\normalsize Conv}$ to obtain the desired result.  The latter is obtained
  by regularity.
  \vspace{-25px}
  \begin{changemargin}{10px}{5px}\noindent
  \begin{itemize}
...
\begin{thm}[Type soundness]\index{Freedom of Speech!Type Soundness}
...
\begin{proof}
  ...
  \item[]Case.\ \\
    ...
    now apply $\FSdrulename{\normalsize AppPiTerm}$ to obtain $\cdot \tvdash{L} S\ e:\nat$.
