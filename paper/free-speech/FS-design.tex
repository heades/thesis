...
there are for types, the type $[[Type]]$, which is only a programmatic
...
type.  This called heterogenous equality.  In addition, note that the
consistency classifiers for the two equations can be different.  For
example, $[[S Z]]$ is logical and programmatic, thus, $[[S Z = S Z]]$
where the first is logical and the second is programmatic is a valid
expression.
