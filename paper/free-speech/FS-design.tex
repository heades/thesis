...
\FSdrulename{ComputeJoin}. This rule states that whenever there exists
...
see the appendix.  Recall from the previous section that the syntax is
...
Suppose the restriction on \FSdrulename{ILam} is lifted. Then the
...
from knowing $[[((x1 : L Nat) + -> z) = ((x1 : L Nat) + -> y)]]$ using only \FSdrulename{Join} and \FSdrulename{Conv}.  So
