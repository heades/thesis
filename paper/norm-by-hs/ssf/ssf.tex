...
Stratified System F (SSF) \index{Stratified System F (SSF)} is
...
type-quantification\index{Polymorphism/Universal Types}.  The types that belong
...
prevents impredicativity\index{Impredicativity}.  That is a type $\forall X.T'$ is no
...
\begin{definition}\index{Ordering on Types}
  The ordering $>_\Gamma$ is defined as the least relation satisfying
...
\begin{definition}\index{Stratified System F (SSF)!Hereditary Substitution Function}
...
definition for STLC\index{Simply Typed $\lambda$-Calculus} (Definition~\ref{def:interpretation_of_types_stlc}),
...
\begin{thm}[Type Soundness]\index{Type Soundness}
...
\begin{corollary}[Normalization]\index{Weak Normalization}
