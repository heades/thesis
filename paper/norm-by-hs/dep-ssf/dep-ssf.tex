...
DSS$\F$ is SSF extended with dependent types\index{Dependent Type} and equations between
terms\index{Equality Type}.  Equations between terms are an important concept in
Martin-L\"of type theory~\cite{hofmann+94,nps90}\index{Martin-L\"of's
  Type Theory (MLTT)}, and play a central
role also in dependently typed programming languages, such as the
\textsc{Guru} language~\cite{stump+09}, $\Sep$\index{Separation of
  Proof from Proof ($\Sep$)}
(Section~\ref{chap:separation_of_proof_from_program}), the freedom of
speech language\index{Freedom of Speech} (Section~\ref{chap:freedom_of_speech}), Coq
